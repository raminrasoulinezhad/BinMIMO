...
void readFileReal(const char* fileName, double** out, int* s1,int* s2, int n, int k) {
	FILE* file = fopen(fileName, "r");
	fscanf(file, "%d\n", s1);
	fscanf(file, "%d\n", s2);
	int dim = (*s2);
	*out  = (double*)malloc( (n)*(k) * sizeof(double));
	for (int i = 0; i < n; i++)
		for (int j = 0; j < k; j++)
			fscanf(file, "%lf\n", &m(*out,i,j,dim));
...
void readFileComplex(const char* fileName,myComplex** out,int* s1,int* s2,  int n, int k) {
	FILE* file = fopen(fileName, "r");
	fscanf(file, "%d\n", s1);
	fscanf(file, "%d\n", s2);
	int dim = (*s2);
	(*out)  = (myComplex*)malloc( (n)*(k) * sizeof(myComplex));
	for (int i = 0; i < n; i++)
		for (int j = 0; j < k; j++) {
			double real, imag;
			fscanf(file, "%lf %lf\n", &real, &imag);
