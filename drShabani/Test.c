...
matrix* matrixMaxScalar(matrix* in, float val) {
...
matrix* matrixMinScalar(matrix* in, float val) {
...
matrix* matrixAddScalar(matrix* in1, float in2) {
	matrix* out = createMatReal(in1->s1, in1->s2);
	long i,j;
	for ( i = 0; i < in1->s1; i++)
		for ( j = 0; j < in1->s2; j++)
...
matrix* matrixMultScalarReal(matrix* in1, float in2) {
	matrix* out = createMatReal(in1->s1, in1->s2);
	long i,j;
	for ( i = 0; i < in1->s1; i++)
		for ( j = 0; j < in1->s2; j++)
...
cmatrix* matrixMultScalarComplex(cmatrix* in1, float in2) {
	cmatrix* out = createMatComplex(in1->s1, in1->s2);
	long i,j;	
	for ( i = 0; i < in1->s1; i++)
		for ( j = 0; j < in1->s2; j++)
...
matrix* matrixPowerReal(matrix* in, float power) {
	matrix* out = createMatReal(in->s1, in->s2);
	long i,j;	
	for ( i = 0; i < in->s1; i++)
		for ( j = 0; j < in->s2; j++)
...
matrix* matrixPowerComplex(cmatrix* in, float power) {
	matrix* out = createMatReal(in->s1, in->s2);
		long i,j;
	for ( i = 0; i < in->s1; i++)
		for ( j = 0; j < in->s2; j++)
...
matrix* loglike(cmatrix* Y, float mean, float var, bool maxSumVal,
...
void estimZ(cmatrix* Y, float var, float mean, cmatrix* PHat, matrix* PVar,
		cmatrix** ZHat, matrix** ZVar) {
	long i,j;	
	matrix* PMonesY_R = sign(matrixAddScalar(realPart(Y), -0.1));
	matrix* PMonesY_I = sign(matrixAddScalar(imagPart(Y), -0.1));
...
void bigamp(opt* opt, cmatrix* A1hatInit, matrix* A1varInit,
		cmatrix* x12hatInit, matrix* x12varInit, cmatrix* pilot, cmatrix* Y,
		cmatrix* x0, cmatrix* mean0, matrix* var0,cmatrix* outcomp, matrix* outreal,int M,int L1,int L2,int N1) {
	long i,j;	
	...
	int it = 0;
	float step = opt->stepInit;
	bool stop = false;
	float step1 = 1;
	float valIn = -INFINITY;
	float valOpt_0 = -INFINITY;
	float valOpt_1 = -INFINITY;
	...
	while(it<=0){
		...
		printf("%f",creal(A1hat->mat[0][0]));
		//pilot2 = matrixPowerComplex(pilot, 2);
		//matrixMultReal(A1var, concat1Real(pilot2, x12hat2));
...
//			float testVal = norm(matrixSubComplex(zhat, zhatOpt)) / norm(zhat); //TODO: define norm
//
...
/*
void structConvert2cmatrix (myComplex* a,cmatrix* b,int s1,int s2){
	int i,j;
	b->s1 = s1;
	b->s2 = s2;
	for (i=0;i<s1;i++)
		for (j=0;j<s2;j++) {
			creal(b->mat[i][j]) = a[i*s2+j].r;
			cimag(b->mat[i][j]) = a[i*s2+j].i;
			}
			
}
void structConvert2matrix (float* a,matrix* b,int s1,int s2){
	int i,j;
	b->s1 = s1;
	b->s2 = s2;
	for (i=0;i<s1;i++)
...
void structConvert2myComplex (cmatrix* b,myComplex* a,int s1,int s2){
	int i,j;
	for (i=0;i<s1;i++)
		for (j=0;j<s2;j++) {
			a[i*s2+j].r = creal(b->mat[i][j]) ;
			a[i*s2+j].i = cimag(b->mat[i][j]);
			}
}
void structConvert2float (matrix* b,float* a,int s1,int s2){
	int i,j;
	for (i=0;i<s1;i++)
		for (j=0;j<s2;j++) {
			a[i*s2+j] = (b->mat[i][j]) ;
...
void cpuKernel2() {
	...
	matrix* vpc = (matrix*)malloc(sizeof(matrix));
	cmatrix* phatc = (cmatrix*)malloc(sizeof(cmatrix));
	
	cmatrix* hhatc = readFileComplex("data//A.txt");
	cmatrix* xhatc = readFileComplex("data/Xdata.txt");
	cmatrix* pilot = readFileComplex("data/Xpilot.txt");
	cmatrix* Y = readFileComplex("data/y.txt");
	cmatrix* h0 = readFileComplex("data/A0.txt");
	cmatrix* Xdata0 = readFileComplex("data/Xdata0.txt");
	matrix* vxc = readFileReal("data/x12var.txt");
	matrix* vhc = readFileReal("data/A1var.txt");
	
	int N = hhatc -> s1;
	int K = hhatc -> s2;
	int pilot_len = K;
	int data_len = xhatc -> s2;
...
//	setting->nitMin = 0;
	...
	matrix* var0 = createMatReal(200,pilot_len);
	cmatrix* mean0 = createMatComplex(200,pilot_len);
	
	cmatrix phat;
	float vp;
	cmatrix* x0 = createMatComplex(1,4);
	x0->mat[0][0] =  0.7071 + I * 0.7071;
	x0->mat[0][1] =  0.7071 - I * 0.7071;
	x0->mat[0][2] = -0.7071 - I * 0.7071;
	x0->mat[0][3] = -0.7071 + I * 0.7071;
	...
	bigamp(setting, h0, vhc, x0, vxc, pilot, Y, Xdata0,
			mean0, var0, phatc, vpc,N,pilot_len,data_len,K);
}  
