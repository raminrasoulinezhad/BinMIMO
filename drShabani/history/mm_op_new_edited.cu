...
void cpuKernelTop_y(float* a1, float* b1, myComplex* c1, float* a2, float* b2, myComplex* c2, int n, int y);
...
float* readFileReal(const char* fileName) ;
myComplex* readFileComplex(const char* fileName);
// =================================================================================
...
int main(int argc, char** argv) {
	...
	myComplex* a = readFileComplex("A.txt");
	//printf("%f",a.r(10));

	//float* b = readFileReal();
...
float* readFileReal(const char* fileName) {
	...
	float* out ;
	n = s2;
	out  = (float*)malloc( s1*s2 * sizeof(float));
	for (float i = 0; i < s1; i++)
		for (float j = 0; j < s2; j++) {
			fscanf(file, "%lf\n", &temp);	///////////////////////
			m(out,i,j) = temp;
		}
	return out;
...
void cpuKernelTop_y(float* a1, float* b1, myComplex* c1, float* a2, float* b2, myComplex* c2, int n, int y) { // one row
	int i=0;
	// myComplex umean;
	// float uvar;
